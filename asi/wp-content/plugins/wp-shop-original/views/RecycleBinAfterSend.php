...
if (isset($shop_id)&&$shop_id!=''&&$secret_key!=''&&isset($secret_key)){
    ...
    $data = array(
      ...
      'DATA'	 => 'Email=' . $this->order['info']['email'] . '|connection_type=wordpress_wp_shop'
    );
    ...
    echo isset($request->PaymentURL) ?
        '<form action="' . $request->PaymentURL . '" method="POST">
             <input type="submit" class="wpshop-button" value="' . __('Pay Tinkoff Bank', 'wp-shop') . '"/>
        </form>' :
        __('The request to Tinkoff Bank is failed', 'wp-shop');
